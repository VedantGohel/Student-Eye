...
def session_live_data(request, pk):
    """
    Returns current processing state as JSON.
    Frontend polls this every 2 seconds during processing.
    """
    from engagement.models import EngagementSession, StudentAttentionScore, FrameEmotionLog

    session = get_object_or_404(EngagementSession, pk=pk)

    # Timeline data — all frames logged so far
    ...
    # Progress percentage
    total_expected = session.frames_processed or 0
